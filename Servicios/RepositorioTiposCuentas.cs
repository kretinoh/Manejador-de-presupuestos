...
namespace ManejoPresupuesto.Servicios
{
    public interface IRepositorioTiposCuentas
    {
        ...
        Task Ordenar(IEnumerable<TipoCuenta> listaOrdenada);
    }
    public class RepositorioTiposCuentas : IRepositorioTiposCuentas
    {
        ...
        public async Task<bool> Existe(string nombre, int usuarioId, int id = 0)
        {
            ...
            var existe = await connection.QueryFirstOrDefaultAsync<int>(
                            ...
                            FROM TiposCuentas 
                            WHERE Nombre = @Nombre 
                            AND UsuarioId = @UsuarioId AND Id <> @id;", new {nombre,usuarioId,id});
            return existe == 1;
        }
        ...
        public async Task<IEnumerable<TipoCuenta>> Obtener(int usuarioId)
        {
            ...
            return await connecion.QueryAsync<TipoCuenta>(
        ...
        public async Task Ordenar(IEnumerable<TipoCuenta> listaOrdenada)
        {
            var query = @"UPDATE TiposCuentas SET Orden = @Orden Where Id = @Id";
            using var connection = new SqlConnection(connectionString);
            await connection.ExecuteAsync(query,listaOrdenada);
        }
    }
}